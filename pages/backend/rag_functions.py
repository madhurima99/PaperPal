...
def prepare_rag_llm(
    token, llm_model, embeddings, vector_store_list, temperature, max_length
):
    # Load embeddings 
    # device = "cuda" if torch.cuda.is_available() else "cpu"
    embeddings = HuggingFaceEmbeddings(
        model_name = embeddings, model_kwargs={"device":"cpu"}
    )

    # Load db
    loaded_db = FAISS.load_local(
        f"vector store/{vector_store_list}", embeddings, allow_dangerous_deserialization=True
    )
